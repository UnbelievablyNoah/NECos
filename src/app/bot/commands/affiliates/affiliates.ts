  ...
  subCommands = [
    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction) => {
        ...
        const pages =
          (affiliates.length > 2 && Math.ceil(affiliates.length / 2)) || 1;
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction<"cached">) => {
        ...
        const dbAffiliates = await database<Affiliate>("affiliates").select(
          "*"
        );
        ...
        for (const affiliate of dbAffiliates) {
          if (
            affiliate.discord_id == guild.id &&
            affiliate.group_id == groupUrlComponents[0]
          ) {
        ...
        if (existingAffiliate)
          return [
            false,
            `An affiliate matching groupId ${groupId} already exists!`,
          ];
        ...
        const representativeRole = guild.roles.cache.find((r) =>
          r.name.includes("Group Representative")
        );
        const errors = [];
        ...
        if (representativeRole) {
          try {
            ...
            errors.push(
              `Failed to add Group Representative role to <@${member.id}>: ${error}`
            );
          }
        }
        ...
        await Interaction.editReply({
          embeds: [
            this.Bot.createEmbed({
              ...
              description: `${
                groupData.name
              } has successfully been linked to this guild.${
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction<"cached">) => {
        ...
        for (const dbAffiliate of dbAffiliates) {
          if (
            dbAffiliate.discord_id == guild.id &&
            dbAffiliate.group_id == groupId.toString()
          ) {
        ...
        if (!affiliate)
          return [
            false,
            `No affiliate was found for this guild matching groupId ${groupId}.`,
          ];
        if (affiliate.owner_id != member.id)
          return [false, "Only the owner of the group can un-link it."];
        ...
        if (representativeRole) {
          for (const representative of representatives) {
            ...
            for (const dbAffiliate of dbAffiliates) {
              const affiliateRepresentatives = JSON.parse(
                dbAffiliate.representatives
              );

              if (affiliateRepresentatives.includes(guildMember.id))
            ...
            try {
              ...
              errors.push(
                `Failed to remove Group Representative role from <@${guildMember.id}>: ${error}`
              );
            }
          }
        }
        ...
        await Interaction.editReply({
          embeds: [
            this.Bot.createEmbed({
              ...
              description: `Your affiliate group has been successfully unlinked from the Discord.${
                (errors.length > 0 && "Errors:\n" + errors.join("\n")) || ""
              }`,
              color: Colors.Green,
            }),
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction) => {
        ...
        if (!affiliate)
          return [false, `No group was found matching groupId ${groupId}.`];
        if (affiliate.owner_id != member.user.id)
          return [
            false,
            `Only the owner of the group can run the setinvite command.`,
          ];

        await database("affiliates").update("invite", invite).where({
          group_id: groupId.toString(),
          discord_id: guild.id,
        });
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction) => {
        ...
        const representative = await guild.members.resolve(
        ...
        if (!representative)
          return [
            false,
            `No user matching userId ${representativeUser.id} could be found.`,
          ];
        if (!representativeRole)
          return [
            false,
            'No group representative role could be found. A group representative role must contain the name "Group Representative".',
          ];
        ...
        if (affiliate.owner_id != member.user.id)
          return [
            false,
            "Only the owner of the group can run the addrep command.",
          ];

        const representatives = JSON.parse(affiliate.representatives);
        if (representative.id == affiliate.owner_id)
          return [
            false,
            "The owner of the group cannot be an extra group representative.",
          ];
        if (representatives.length == 2)
          return [
            false,
            "Affiliate Groups can have no more than two extra group representatives.",
          ];
        if (representatives.includes(representative.id))
          return [
            false,
            `<@${representative.id}> is already a group representative.`,
          ];
        ...
        try {
          ...
          errors.push(
            `Failed to add Group Representative role to <@${representative.id}>: ${error}`
          );
        }
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction) => {
        ...
        if (!affiliate)
          return [
            false,
            `No affiliate could be found matching groupId ${groupId}.`,
          ];
...

    {
      ...
      onCommand: async (Interaction: ChatInputCommandInteraction) => {
        ...
        const representatives: Array<string> = JSON.parse(
          affiliate.representatives
        );
        if (!representatives.includes(representative.id))
          return [
            false,
            `<@${representative.id}> is not a group representative.`,
          ];
        ...
        try {
          ...
          errors.push(
            `Failed to remove Group Representative role to <@${representative.id}>: ${error}`
          );
        }
